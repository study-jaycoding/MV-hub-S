...
REM  local PC can open it at  http://<server-ip>:8011  (use open-test.bat).
REM
REM  Isolated from the live service (which stays on 8010):
REM    - PORT 8011
REM    - data = THIS folder's backend\data  (a COPY of live; use
REM             refresh-server-test-db.bat to snapshot the live DB into here)
...
REM    1) refresh-server-test-db.bat   (copy live DB -> this test DB)
REM    2) run-server-test.bat          (this file)   -> serves 8011
...
echo             open from your PC: http://^<server-ip^>:%PORT%   (or use open-test.bat)
echo.
