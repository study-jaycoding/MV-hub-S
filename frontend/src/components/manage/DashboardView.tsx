...
export function DashboardView() {
  ...
  // 담당 배정/해제 — 선택 시퀀스의 작업(들)에 배치 1회로 적용 후 재로딩.
  // 백엔드가 권한을 검사하고 한 트랜잭션으로 처리한다. 실패해도 reload 로 실제 상태를 맞춘다.
  ...
  const unassign = (taskIds: string[], uid: string) =>
    manageApi
      .bulkSetAssignments(taskIds.map((t) => ({ task_id: t, assignee_uids: [uid] })), "remove")
      .then(reload)
      .catch((e) => {
