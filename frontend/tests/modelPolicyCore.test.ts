...
describe("restrictedSet — 내용이 같으면 같은 Set 을 돌려준다", () => {
  it("정리(소문자·공백·중복)하고, 같으면 이전 참조 유지", () => {
    const first = restrictedSet([" Seedance_2_5", "seedance_2_5", "gpt_image_2"]);
    expect([...first]).toEqual(["seedance_2_5", "gpt_image_2"]);
    expect(restrictedSet(["gpt_image_2", "seedance_2_5"], first)).toBe(first);
    expect(restrictedSet([], first)).toBe(EMPTY_RESTRICTED);
    expect(restrictedSet(null)).toBe(EMPTY_RESTRICTED);
  });
});
...
describe("상태 전이 — 캐시·응답·실패·늦은 응답", () => {
  ...
  it("키 없음 = 정책 없음, 캐시 없음 = loading, 캐시 있음 = stale(그 값으로 바로 거른다)", () => {
    ...
    const stale = initialState(key, { restricted: ["seedance_2_5"], groupName: "Artist", revision: 3, fetchedAt: 10 });
    expect(stale.status).toBe("stale");
    expect(stale.restricted.has("seedance_2_5")).toBe(true);
  ...
  it("응답은 같은 키에만 반영되고 캐시로 남는다", () => {
    ...
    const ready = appliedResponse(loading, key, { restricted_models: ["gpt_image_2"], group_name: "TD", revision: 5 }, 100);
    expect(ready.status).toBe("ready");
    expect(ready.groupName).toBe("TD");
    expect(cacheOf(ready)).toEqual({ restricted: ["gpt_image_2"], groupName: "TD", revision: 5, fetchedAt: 100 });
    // 다른 키(공간 A→B→A 전환 중 옛 공간 응답)는 무시
    expect(appliedResponse(ready, "srv|a@x|ws2", { restricted_models: [] }, 200)).toBe(ready);
  ...
  it("구서버는 제한 없음으로 확정, 권한·통신 실패는 캐시 값을 지키되 미확인(stale/error)으로 표시", () => {
    ...
    expect(unsupported.restricted.size).toBe(0);
    const auth = appliedFailure(stale, key, "auth");
    expect(auth.status).toBe("stale");
    expect(auth.restricted.has("seedance_2_5")).toBe(true);
...
describe("submitBlockMessage — 제출 직전 가드", () => {
  const key = "srv|a@x|ws1";
  it("첫 조회 중이면 보류, 제한 모델이면 그룹 이름과 함께 막고, 그 밖엔 통과", () => {
    expect(submitBlockMessage(initialState(key, null), "gpt_image_2")).toMatch(/확인하는 중/);
    const ready = appliedResponse(initialState(key, null), key, { restricted_models: ["gpt_image_2"], group_name: "Artist" }, 1);
    expect(submitBlockMessage(ready, "gpt_image_2", "GPT Image 2")).toBe(
      "GPT Image 2 모델은 'Artist' 그룹에서 제한되어 있습니다. 다른 모델을 고르세요.",
    );
  ...
  it("다른 공간의 요청(복구 재실행)에는 지금 정책을 적용하지 않는다", () => {
    ...
    expect(submitBlockMessage(ready, "gpt_image_2", undefined, "ws1")).toMatch(/제한되어/); // 같은 공간 = 검사
    expect(submitBlockMessage(ready, "gpt_image_2", undefined, "ws2")).toBeNull(); // 다른 공간 = 검사 안 함
    expect(submitBlockMessage(ready, "gpt_image_2", undefined, null)).toMatch(/제한되어/); // 모르면 지금 정책으로
...
describe("policyNote — 정책을 확신할 수 없을 때만 알린다", () => {
  ...
  it("stale·error 는 문구, ready·unsupported·none 은 없음", () => {
    const stale = initialState(key, { restricted: [], groupName: null, revision: 1, fetchedAt: 5 });
    ...
    expect(policyNote(appliedResponse(initialState(key, null), key, { restricted_models: [] }, 1))).toBeNull();
