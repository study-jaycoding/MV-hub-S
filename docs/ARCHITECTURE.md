...
> [!NOTE]
> 아래 표는 흐름 설명에 필요한 대표 파일만 싣는다. `lib/` 전체(약 210개)와 `components/`·백엔드·스크립트의
> 파일 단위 색인은 [CODE_MAP.md](CODE_MAP.md) 에 있다.
...
- **Media Pool 선택은 기존 MV Hub 캔버스의 결과 창에 표시한다.** 로컬 작업자 허브와 격리 `test_dev`가 브라우저
  WebSocket 연결이 있을 때만 `resolve_selection_worker.py`를 호환 Python 자식으로 띄워 기본 0.05초 대기 간격으로
  `GetSelectedClips()`와 프로젝트·미디어 ID를 읽는다. 선택이 바뀔 때만 파일 경로·메타데이터를
  조회하며 미디어 ID를 중복 조회하지 않는다. 연결/프로젝트가 없으면 최소 0.5초 대기한다.
  별도 창은 없고, 서버 프로세스는 `fusionscript`를 직접 로드하지 않는다.
  새로 반입한 클립에는 third-party metadata `MV Hub`로 `generation_id`·`project_id`만 각인한다.
  과거 클립은 현재 계정 프로젝트의 검증된 전송 manifest에 기록된 **원본 절대 경로 완전 일치**만
  폴백으로 허용하며 파일명 추측은 금지한다. 일반 미디어 경로는 manifest를 훑지 않고, 과거 인덱스는
  60초 재사용·직접 전송 뒤 즉시 무효화·프로젝트당 최신 5천 전송 상한을 적용한다. 선택 신호는 현재
  계정 WebSocket에 정렬·중복 제거한 `generation_ids`와 원본 `selected_count`를 전송한다.
  구형 소비자용 `generation_id`는 첫 ID 또는 null이며, 명시된 빈 배열은 선택 해제다 —
  미디어 바이트·프롬프트·로컬 경로는 브라우저나 공유 서버로 보내지 않는다.
- `test_dev`는 AUTH-on 격리 실행이라 머신 `active.json` 대신 `/me`가 확인한 로컬 pairing 계정을
  사용하고, 그 계정이 볼 수 있는 프로젝트만 인덱싱한다. pairing 계정이 없거나 승인 계정이 아니면
  아무 신호도 보내지 않는다.
- 프런트는 마지막으로 사용한 보이는 MV Hub 탭 하나만 반응한다. **현재 탭을 유지한다.** 캔버스에서는 현재 캔버스 → 생성물을 담은 가장
  최근 캔버스 순으로 찾고, 이미 열린 결과 카드 → 목표가 대표인 카드 → 첫 일치 카드 순으로 기존
  변형 팝업을 연다. **복수 선택은 이미 열린 결과 창을 유지하고 그 안의 일치 항목을 모두 표시한다.**
  교집합이 없어도 다른 묶음으로 이동하지 않고 Resolve 강조만 지운다. 열린 창이 없으면 위 우선순위로
  한 묶음만 연다. 목표는 빨간 테두리와 글로우로 선택하되 **대표 생성물과 Last viewed는 바꾸지 않는다.** Resolve
  가져오기 중에는 읽기 자식을 멈춰 같은 프로젝트 API를 동시에 호출하지 않는다. 자식 시작 직후의
  현재 선택은 기준선으로만 삼아 브라우저 재연결·전송 완료만으로 화면이 움직이지 않게 한다.
  선택 처리 중 DB·NAS·WebSocket 일시 오류가 나면 자식을 정리하고 백오프 후 다시 시작해, 오류 한 번으로
  서버 재시작 전까지 감시가 영구 정지하지 않게 한다.
- 부모는 파이프 읽기와 매핑을 분리한다. 읽기 쪽은 첫 기준선을 먼저 소비하고 최신 신호 한 개만
  슬롯에 남긴다. 슬롯 교환에는 await가 없으며, 단일 매핑 작업이 끝난 뒤 revision을 재확인해
  새 선택·선택 해제·EOF·전송 중단 전에 받은 낡은 결과는 방송하지 않는다. 선택마다 스레드를
  취소하거나 DB/NAS 매핑을 병렬 실행하지 않으며, 종료 시에는 캐시를 쓰는 스레드 완료까지 기다린다.
  MV Hub에서 직접 선택한 항목은 기존 라임 링만 쓰며 Resolve 선택에만 빨간 링과 글로우를 적용한다.
  라이브러리의 빨간 링은 카드 부모의 독립 레이어로 즉시 적용·해제한다. 일반 카드의 라임/hover 전환은
  유지하며, 브라우저 창 가림에 따른 프레임 스로틀까지 해제하는 것은 아니다.
- 복수 감시는 최대 200개만 상세 조회·매핑한다. 자식 JSONL은 900KiB 이내로 줄이고 부모 reader는
  1MiB를 허용한다. 개수·길이 상한에 걸리면 원본 선택 수를 유지하며 `truncated`로 표시한다.
  이때 자동 창 열기·탭/씬 이동 없이 현재 창과의 교집합만 표시한다. 매핑은 한 계정 스냅샷으로
  순차 실행하며 중간에 계정이 바뀌면 묶음 전체를 버린다. 경로는 로컬 자식→부모 검증에만 쓴다.
- 작업 공간·공유 & 리뷰에서는 읽기 전용 `POST /api/generations/locate`로 실제 프로젝트/폴더와 대상
  메타(최대 200개)를 찾는다. 공유 탭은 서버의 현재 공유·접근권한·휴지통 조건을 따르며 비공유·삭제·
  접근 불가·구서버/네트워크 오류는 이동·필터 해제·알림 없이 이전 Resolve 강조만 지운다. 로컬 정보로
  대신 열지 않는다. 복수 선택은 현재 프로젝트/폴더(하위 포함)를 우선하고, 없으면 가장 많은 대상의
  한 폴더(동률은 최신 항목)를 표시한다. 폴더가 다른 PC에만 있어도 읽기 전용 경로로 표시한다.
  유효한 대상일 때 검색·유형·컬러·태그·전역태그·워크스페이스 칩·작성자·공유 방향·공유/최종/코멘트/
  회색 보기 필터를 해제한다. `generationScopeV1`/`armedFolder`/`armedAutoTags`는 생성용으로 보존하고,
  조회용 `filterAutoTagsV1`과 구분한다. 자동 확인은 대표·Last viewed·일괄 작업 선택을 변경하지 않는다.
  첫 페이지 밖 대상도 정렬 순서에 맞게 합치지만 커서·hasMore는 정상 페이지로만 계산한다. 자동 스크롤은
  후속 페이지 연속 조회를 막으며 사용자 스크롤부터 복귀한다. 재조회 때 공유·삭제·이동을 재검증한다.
  선택 시 서버 locate는 항상 수행한다. 같은 필터 없는 폴더에서 모든 대상이 이미 표시되고, 정확히 같은
  쿼리의 정상 페이지 캐시가 15초 이내이며, 이전 목록 탭이 일치하고 진행/대기/추가 로드·오류가 없을 때만
  선택 직후의 추가 전체 재조회를 생략한다. 필터 해제/폴더 변경이 필요하면 재사용하지 않는다.
  캐시 시각·다음 페이지 커서·hasMore와 정상 갱신의 locate 재검증은 그대로 유지한다.
  계정/서버/탭/수동 필터 변경은 대기 선택을 폐기하고, 계정/서버 변경은 목록 캐시·주입 대상도 비운다.
- 설정 → 외부 프로그램 → DaVinci Resolve의 **실시간 체크**는 기본 체크다. 체크하면 위 자동 이동·열기를
  수행하고, 해제하면 현재 캔버스의 이미 열린 결과 팝업 또는 현재 라이브러리 목록에 포함된 생성물만 표시한다. 일치 항목이 없으면
  단일·복수 모두 이전 Resolve 강조만 지우고 직접 선택은 유지한다. 닫힌 팝업·다른 카드·다른 탭으로
  이동하거나 나중에 창을 열 때 신호를 재생하지 않는다. 설정은 브라우저에 저장하며 같은 창과
  다른 탭에 즉시 반영한다. 설정 변경 시 대기 선택도 취소한다. 수신 당시 설정 `followEnabled`와
  실제 자동 열기 `openPopup`를 분리해 빈 배열·상한 초과 신호와 설정 변경 직전 신호를 구별한다.
  첫 전환은 팝업이 열린 렌더를 확인한 뒤 선택 신호를 소비한다. 초기화 effect → 선택 effect 순서로
  처리해 StrictMode 재실행에서도 첫 선택이 지워지지 않는다. 씬 교체 직후의 이전 cards는 수용하지 않는다.
...
- **전송 파일명 = `<폴더 경로를 _ 로 이음>_<번호 2자리>.<확장자>`** (`services/resolve_transfer.py` 의
  `_transfer_name`·`_place_copy`). 예: `e020/c0020` 의 첫 컷 → `e020_c0020_00.mp4`. 번호의 **유일한 근거는 그 컷
  폴더**다 — 따로 적어 두는 기록이 없다. 규칙은 한 줄이다: **남아 있는 `<base>_NN` 중 마지막 번호 + 1.** 가운데 빈
  번호는 채우지 않고, 폴더를 통째로 비웠으면 00 부터 다시 센다. 같은 생성물을 또 보내도 똑같이 다음 번호를 받는다
  (사본이 하나 더 생긴다).
  2026-09-02 Jay 결정으로 두 가지가 폐기됐다 — `@davinci/.mvhub/sequence.json` **번호 장부**(어느 생성물이 몇 번인지
  기억)와 **알파벳 꼬리**(`_00_a` = 이미 가져온 것 표시). 둘 다 "같은 생성물을 또 가져왔다" 를 표시하려던 장치이고,
  그냥 다음 번호로 들어오면 되므로 필요 없어졌다. 종전의 바이트 대조 후 `skipped` 멱등도 함께 폐기됐다.
- 대상 폴더를 **읽지 못하면** 빈 폴더로 보고 00 부터 세지 않고 실패로 남긴다(`_scan_numbered`) — 그러지 않으면 기존
  파일 번호를 다시 써서 덮어쓸 뻔한 이름을 만든다. 전송 한 건 안에서는 폴더 번호를 캐시해 NAS 재스캔을 피하고, 방금 쓴
  번호를 캐시에 넣어 다음 항목이 그 다음 번호를 받는다. `_seq_in_name` 은 옛 꼬리(`_07_a`)가 붙은 파일의 번호도 읽는다 —
  규칙을 바꾸기 전에 만든 파일이 디스크에 남아 있어 그 번호를 다시 쓰지 않으려면 같이 세야 한다.
- **스틸 이미지는 한 호출에 하나씩 반입한다**(`resolve_bridge._import_media_items`, Resolve 메뉴 스크립트도 동일).
  `ImportMedia` 에 번호만 다른 이미지를 한꺼번에 넘기면 Resolve 의 이미지 시퀀스 자동 인식이 클립 하나(`[00-01]`)로
  묶는다 — 번호 규칙이 시퀀스 패턴과 똑같이 생겼기 때문이다. 판정은 manifest 필드가 아니라 **확장자**로 한다
  (`_STILL_IMAGE_SUFFIXES`) — 반입 계층까지 내려오는 항목에는 `media_type` 이 없다. 영상·오디오는 시퀀스로 합쳐질 일이
  없어 종전대로 묶어 넣는다(대량 전송 성능). ★`ImportMedia([{"FilePath": …}])`(clipInfo) 는 README 상 항목당 클립
  하나지만 실기에서 **아무것도 가져오지 못했다**(2026-09-02 실측, `media_import_failed`) — 쓰지 말 것.
- 복사는 기존 파일을 **어떤 경우에도 덮지 않는다**. 순번 이름은 종전 `<컷>_<gen 해시12자>` 와 달리 다른 PC 가 같은
  이름을 고를 수 있어서, 복사 전에 `O_CREAT|O_EXCL` 로 이름을 원자적으로 선점하고 실패하면 다음 꼬리로 넘어간다
  (선점만 하고 복사에 실패하면 빈 파일을 지워 이름을 되돌린다). 같은 번호를 다른 전송이 먼저 가져갔으면 다음 번호로
  넘어간다. 2026-09-02 이전에 전송한 파일은 옛 해시 이름 그대로 남는다 — 새 규칙은 새로 쓰는 파일에만 적용된다.
  > **알려진 한계(2026-09-02 — 현행 유지).** 이름 선점 직후(0바이트 상태) 프로세스가 죽으면 빈 파일이 남고 그 번호가
  > 쓰인 것으로 계산된다. 동시 전송에서만 생기고 결과는 번호 하나를 건너뛰는 정도다.
- Windows 경로 안전 비교와 대상 파일 잠금 키는 `path_safety.path_comparison_key`를 함께 쓴다.
  양쪽 경로의 `resolve()`는 유지하고, 알려진 `\\?\C:\...`·`\\?\UNC\서버\공유\...` 표기만 비교용으로
  일반 드라이브·UNC와 통일한다. 컴포넌트 단위 포함 검사이며, 실제 파일 IO에는 원래 해석된 Path를
  그대로 사용한다. 해석 뒤에도 기준/후보에 `..`가 남으면 거부한다. 긴 경로 지원·링크 이탈 거부·
  기존 파일 비덮어쓰기 계약은 바꾸지 않는다.
...
> 기록한 뒤 활동을 **재확인**한다. 수동 업데이트까지 원자적으로 보호하기 위해 Windows 릴리스에서는
> `resolve_transfer_gate.transfer_lease`가 설치 루트의 `.resolve-transfer.lock`을 공유 모드로 연다.
> 부모는 준비·반입·완료 저장까지, 자식은 실제 반입·journal 완료까지 별도의 비상속 핸들을 유지한다.
> 다른 위치에 설치된 호환 Python도 모듈 파일의 설치 루트를 사용하므로 같은 보호에 참여한다.
> 업데이터는 기존 `.update.lock` 뒤에 이 파일을 배타적으로 열고 종료까지 유지한다. 여러 전송은 허용하되
> 전송과 업데이트는 양방향으로 배제하며, 선택 감시 자식은 참여하지 않는다. 잠금 파일의 존재·나이는
> busy 판정이 아니다. 닫힌 빈 파일은 그대로 두며, 실행기 교체 중 설치 표식이 잠시 없어도 기존 파일로
> 보호를 유지한다. 권한/디스크 오류는 busy로 숨기지 않고 전송을 시작하지 않는다(HTTP 503).
> 업데이터 중복은 기존 17/상태 보존, 전송 busy는 별도 18/재시도 가능한 `available` 안내로 구분한다.
> 기존 `recovery_required`는 덮지 않는다. **최초 적용은 전송 완료 → 옛 MV Hub 완전 종료 → 수동 업데이트 →
> MV Hub 재시작** 순서다. 옛 프로세스는 새 잠금에 참여하지 않으므로 기존 설치 루트 기반 자식 검사도
> 유지한다. Resolve 자체를 닫을 필요는 없다. 알려진 한계 두 가지(2026-08-27 Jay 결정으로
> 현행 유지): ① Resolve 자식 반입에 timeout 이 없어 자식이 멈추면 보호가 유지돼 업데이트가 막힌다(`_IMPORT_LOCK` 과
> 같은 성질). 강제 업데이트도 OS 핸들을 우회하지 않으므로 해당 Python 자식의 작업·복구 상태를 확인해야 한다.
> 사용자 작업을 보호하기 위해 자식이나 Resolve를 자동 종료하지 않는다.
> ② 허브 직접 반입은 프로세스 안 `_IMPORT_LOCK` 만 쓰고 Resolve 메뉴 Importer 의 파일 락에는 참여하지
...
11. **썸네일 캐시는 서버·브라우저 두 겹이고, `v`(파일 버전)는 브라우저 쪽만 담당한다** — 함정.
    서버 디스크 캐시는 `services/thumbs.py` 의 `cache_path` 가 `경로|mtime_ns-size|폭` 해시로
    키를 잡아 **덮어쓰면 자동으로 새 키**가 된다(초 단위 mtime 은 1초 안 덮어쓰기를 놓치므로
    나노초+크기를 쓴다). 브라우저 쪽이 문제다 —
