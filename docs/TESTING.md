...
| 파일 | 실행 위치 | 하는 일 |
|---|---|---|
| `test_dev.bat` | 내 PC | 한 번에 테스트 백엔드(8012)·생성 에이전트·Vite(5173)를 실행하고 실시간 프론트엔드를 엶 |
...
최신 운영 DB 모양이 필요할 때만 먼저 `test_pull-db.bat`을 실행한다. 내려받은 DB와 이후
생성 결과는 `backend\data_test`에만 저장되며 운영 DB에는 쓰지 않는다. 단, 생성 요청은
실제 Higgsfield 작업이므로 실제 크레딧을 사용한다.
