...
합격 조건은 세 파일 전부 통과다(2026-09-09 기준 67개 — 테스트가 늘면 그 수). 여기에는 빈 `TEMP`, 공백이 있는 설치 경로, 실행 중 작업자 교체,
...
`test_pull-db.bat`의 서버 주소 기본값은 `http://192.168.1.199:8011`이다. 서버가 이사했으면 실행 전에
환경변수 `MVHUB_SNAPSHOT_SERVER`(예: `set MVHUB_SNAPSHOT_SERVER=http://새주소:8011`)로 바꾼다. 명령줄 인자는
일부러 받지 않는다(`%~1`은 파싱 시점에 확장돼 명령 주입을 막을 수 없다). `http://호스트:포트` 형식만 받으며
https·포트 생략·경로·따옴표·공백이 섞인 값은 `tools\refresh_pm_test_data.py`가 종료코드 2로 거부한다.
...
같은 저장소의 이전 `test_dev.bat` 세션은 새 런처가 **자동으로 종료·교체**한다(`tools/replace_dev_session.ps1`).
5173 또는 8012를 **다른 프로그램**이 쥐고 있을 때만 중복 생성 에이전트를 만들지 않고 안내 후 종료한다(보조 스크립트 exit 2 → 런처 최종 `exit /b 1`).
...
`smoke.py` 의 종료 코드: **0** 전부 통과 · **1** 제품 쪽 확인 필요(기대와 다름·예외·생성 요청 발생) · **2** 시나리오만 낡음(대상을 찾지 못함 —
화면이 바뀐 것이니 제품 결함으로 읽지 말고 `smoke.py` 를 고친다).
...
자동화 요령(2026-09-21): ① `cdp.Page.click` 은 대상이 화면 밖이면 스크롤 → 0.45초 대기 → 좌표를 다시 잰다 — 스크롤 직후의 좌표는 가상 목록이 자리를 다시
잡으면서 어긋나 **새 줄 첫 카드의 클릭이 빈 곳에 떨어지고 '선택이 안 된다'처럼 보인다**(제품 결함으로 읽지 말 것) ② HTML5 끌기(카드 → 프로젝트 행, PM 보드)는
페이지 안에서 `new DataTransfer()` 와 합성 `DragEvent`(dragstart → dragover → drop)로 앱의 드롭 처리를 검증한다(브라우저의 끌기 개시는 범위 밖)
③ **화면 위치·크기를 다루는 수정은 창 있는 크롬으로도 한 번 본다** — 헤드리스는 열리는 애니메이션 도중의 측정 오차를 가려 준 적이 있다(`--headless=new` 만 빼고
전용 프로필로 띄우면 된다, Jay 의 Chrome 과 별개 프로세스) ④ Codex 와 동시에 재려면 `--work <다른 폴더> --hub-port 8241 --server-port 8242`.

격리 환경 안에서 끝나 **재도 되는 쓰기**(2026-09-19 Codex 가 소스로 확인): 영구 삭제 · 관리자 창의 계정 승인/전역 역할 · 업데이트 공지 작성 ·
공유 서버 이름·주소 저장 · PM 의 작업·프로젝트·크레딧 플랜/그룹 한도/허용 모델 저장 · 설정의 강조색·언어(브라우저 프로필에만 남는다).
**누르지 않는 것에 추가**: 관리자 창의 **팀에 공지**(릴리스 폴더에 쓰고 자식 프로세스를 띄울 수 있다) · 설정의 **DB 내보내기**(임시 파일·다운로드가
격리 폴더 밖에 남는다).
