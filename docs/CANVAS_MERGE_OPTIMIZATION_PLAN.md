...
> Phase 4의 드래그 세션 생명주기, 카메라·좌표 계산, 카드 크기 조절, 카드 이동과
> 그룹 이동(`useSceneCardResize`, `useSceneCardMove`, `useSceneGroupMove`), Phase 5의 실행 경계
> (`sceneComfyExecutor` + `useSceneComfyExecution`)를 분리했다. 카드 상태 소유자는 여전히
> `SceneBoard` 하나다. 다음 작업은 Phase 4의 배경 마퀴 포인터 판정을 작은 단위로
