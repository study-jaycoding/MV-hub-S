...
- `SceneBoard.tsx` 3,507줄(카드·입력·카메라·이동 훅 분리), repo `generations.py` 832줄,
...
- 다중 담당 해제, Assets 색상·태그·소스, PM 작업 순서·삭제, 라이브러리 색상·태그의
  ...
  기능 의미인 경로다.
- P18~P27 자동 검증은 백엔드 346개·프론트 299개, TypeScript/Vite 빌드와 ESLint 구조 검사를
...
- 남은 `id/job_id` 데이터 마이그레이션은 구버전 데이터 호환이 선행돼야 하는 별도 고위험 단계다.
  ...
  크기 상한·실행 중 워크플로 교체 가드·미디어 최종 실패 표시·영상 비교 seek 동기화는 구조가 아니라
  별도 기능·운영 정책이다.
