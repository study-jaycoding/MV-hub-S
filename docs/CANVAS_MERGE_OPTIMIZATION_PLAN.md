...
- `assets.py`의 디스크 IO·계정별 마운트 저장·개인 메타·팀 코멘트를 하위 서비스와 라우터로
  분리해 1,225줄에서 851줄로 줄였다. 여러 Assets 창의 동시 마운트 등록도 변경을 잃지 않는다.
- 백엔드 전체 import 순환 0건, 동일 AST 함수 복제 0건, 계층 역방향 import 0건이다.
- 남은 `id/job_id` 데이터 마이그레이션은 구버전 데이터 호환이 선행돼야 하는 별도 고위험 단계다.
  Assets의 남은 파일 라우트는 같은 로컬 디스크 경계를 공유하며 현재 성능 병목은 아니다. Comfy 상한·
  미디어 fallback·영상 seek는 구조가 아니라 기능 정책이다.
