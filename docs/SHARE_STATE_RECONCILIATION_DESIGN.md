...
- 사용자 후속 요청으로 처음의 가로 2개/취소 버튼 디자인을 대체한다.
  ...
  선택 전후 padding 1px·배치는 고정이며, 선택 시 클립을 3px 더 안으로 잘라 4px 프레임을 드러낸다(2026-09-21 Jay: 3px → 4px).
  바깥 반경 14px = 여백 1px + 선택 클립 3px + 안쪽 반경 10px로 모서리 중심을 맞춘다.
  키보드 포커스는 흰 링+라임 링 대신 **자기 상태색 테두리가 밝아지는 것**으로 표시한다(2026-09-21). Resolve 강조는 기존 별도 표시(부모 `::after` 의 빨강 링·빛, 최우선 오버레이)를 유지한다.
...
- 보류 버튼과 모든 보류 S는 `#c9384a` 바탕·흰 글씨. 기존 라임 S와 골드 최종 스타일 유지.
  ~~카드 테두리는 바꾸지 않는다~~ → **2026-09-21 Jay 결정으로 뒤집음**: 카드 테두리가 상태색을 따른다(공유 라임·보류 `#c9384a`·최종 골드·실패 빨강, 평소 1px 선·선택 4px,
  방금 내가 바꾼 카드는 선택 전까지 같은 색의 멈춘 빛 — `styles/generations.css` "테두리의 상태색", `lib/stateGlow.ts`). 다빈치 확인의 빨강(`#ff304f` 2px 링+강한 빛)은
  더 밝고 굵은 별도 오버레이라 보류 빨강 위에 그대로 덮여 보인다.
