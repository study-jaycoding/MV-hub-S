...
14. **Comfy 실행 동일성** — 실행 시작 때 `content`와 노출 파라미터를 스냅샷으로 고정한다. 미디어
    다운로드·API 실행·자동 저장 중 현재 카드 입력이 바뀌면 옛 결과·실패·`genId`를 새 카드에 쓰지 않고
    하류 생성도 중단한다. 이미 원격에서 시작된 작업 자체는 취소하지 못한다.
15. **Comfy 그래프 입력 동일성** — 카드·엣지 전체를 단순 비교하지 않고 실제 API 입력인 정렬된
    미디어 URL·타입·출처와 최종 텍스트/파라미터만 비교한다. 실행 중 이 값이 바뀌면 결과·실패·하류
    생성을 폐기하며, 자동 저장은 현재 그래프가 아니라 실행 당시 입력·무작위 seed를 출처로 기록한다.
