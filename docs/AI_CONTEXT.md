...
- **실행**: `run-server.bat`(프론트 빌드 → 백엔드 기동). 기본 **포트 8010**, **로그인 강제 ON**(`CONTENT_HUB_AUTH=1`, bat 기본값). `serve.py` 가 IPv4 0.0.0.0 + IPv6 ::1 듀얼스택(Windows localhost IPv6 폴백 ~200ms 지연 제거).
...
## 6. push 에이전트 (`push_agent.py`) + 적재(ingest)

`content-hub-server/push_agent.py` — **표준 라이브러리만**(팀원 무설치). 각 PC에서 실행:
```
python push_agent.py --server http://<서버IP>:8010 --email <내이메일> [--watch 30]
...
- `run-server.bat` 기본 = 포트 **8010** + 로그인 **ON**(`CONTENT_HUB_AUTH=1`). 끄려면 bat 에서 0.
