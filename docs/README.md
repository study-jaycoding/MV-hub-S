...
4. **팀 공유** 탭 → ⬇ 가져오기 = 내 워크스페이스로 복제 + lineage 기록.
...
- **CLI 필드 매핑**: `generate list --json` 실제 출력으로 검증 —
  `id→PK`, `job_set_type→model`, `result_url`(확장자로 image/video),
  `created_at`(epoch→ISO), `params.prompt`, `params.medias[]→reference`.
