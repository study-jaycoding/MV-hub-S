...
기준일: **2026-08-27** · 코드 기준선: `dev` `5f7a1853` (팀 릴리스·`origin/main` 은 `aa0985b9` — 코드 차이는 `c243b211` 브라우저 유지·`fe1efacf` 큐 host_id 둘뿐) · 운영 배포 검증 전
...
- **Resolve 전송 = 직접 전송이 현행.** 서버 영구 큐 없이 **요청 한 건 안에서 준비·반입·결과 저장**을 끝낸다
  ...
  큐 v3(08-23~24)는 **이력**이며 현재 확인된 재도입 계획은 없다. 다만 `services/resolve_queue*.py`·`resolve_lock.py`·
  `resolve_import_worker.py` 와 라우트 3개(`/api/resolve/queue*`)·`release_update` 의 큐 스캔은 **비현행 코드로 잔존**한다
  (워커 비활성 — 라우트·스캔은 호출되지만 처리되지 않음). 제거는 코드 결정이다.
...
> [!IMPORTANT]
> 아래 숫자는 2026-08-27 `dev` `5f7a1853` 에서 격리 환경(`CONTENT_HUB_NO_PROXY=1`, 임시 DB)으로 실행한 결과다.
> 이후 커밋의 전체 회귀를 자동으로 보증하지 않으며, 새 릴리스 전에는 Gate 6 절차로 다시 확인한다.
> Resolve·이사 공지 행은 2026-08-24 실측 기록이다(Resolve 는 당시 큐 v3 경로 — 직접 전송 경로의 실기기 재실측은 아직 없다).

| 검증 | 결과 |
|---|---|
| 백엔드 전체 테스트 | **1,854개 통과 + 29 subtests** (3분 29초) |
| 프론트 전체 테스트 | **98개 파일·679개 통과** · `lint:architecture` 이상 없음 · `build` 성공 |
...
1. **새 릴리스 제작과 운영 설치본 확인**
   ...
   `c243b211`(런처가 브라우저를 안 닫음)·`fe1efacf`(큐 host_id — 워커는 비활성이라 전송 동작은 그대로지만, 노출된 `/api/resolve/queue*` 조회·취소·재개 응답에는 host 필터가 적용된다)다. 작업자 PC는 앱 안
   **설정 → 프로그램 업데이트**로 받는다.
