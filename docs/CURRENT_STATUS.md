...
- **그룹별 제한 모델(2026-09-11, Jay 요청 · Codex 설계 검토 1 + 코드 리뷰 2).** 크레딧 그룹 창에 "제한 모델"(힉스필드 User Group →
  Restricted Models 와 같은 차단 목록)을 넣고, 그 그룹 멤버의 **생성 창 모델 드롭다운·캔버스 모델 설정 노드**에서 그 모델을 숨긴다.
  이미 골라 둔 모델은 자동으로 바꾸지 않고 '제한됨' 표시 + 제출만 막는다(Jay 결정). **힉스필드 CLI·개발자 API 는 워크스페이스별
  허용 모델을 주지 않는다**(5개 워크스페이스 실측 동일) — 크레딧 풀처럼 손 입력이고 강제는 힉스필드가 한다.
  저장 계약의 핵심은 **필드가 없으면 기존값 유지**(구버전 앱·'추정' 맞추기가 제한을 지우지 않게). 정책은 앱에서 한 번만 구독하고
