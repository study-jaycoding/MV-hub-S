...
1. **`2026.09.10-1322` 배포 뒤 남은 것** — 공지는 09-10 13:35 발표됨. 남은 건 ①크레딧 그룹 입력(워크스페이스 5곳 모두 그룹 0개, 뻘뻘뻘만 플랜 있음) ②생성 일시중지 스위치를 켰다면 서버 PC 에서 GET 으로 확인·해제 ③Gate 6-C 실측(아래 절). 아래 `1014` 항목은 이전 릴리스 기록.
