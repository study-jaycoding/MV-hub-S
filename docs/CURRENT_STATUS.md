...
기준일: **2026-08-27** · 코드 기준선: `dev` `8f6f9cbb` (팀 릴리스·`origin/main` 은 `aa0985b9` — 코드 차이는 `c243b211` 브라우저 유지·`fe1efacf` 큐 host_id·2026-08-27 코드 정리 7커밋 `f78eafe4`~`3fa4e862`·`38bb8e73` test_dev 포트 env·`8f6f9cbb` 캔버스 렌더 노드 하단 모델 폴백) · 운영 배포 검증 전
...
> 들어가 `test_dev.bat` 의 Vite 가 `EACCES` 로 못 뜬다 — 환경 문제(코드 아님). `38bb8e73` 부터 실행 전 `$env:FRONTEND_PORT=3173`
> 처럼 env 로 덮어쓸 수 있다(기본값 5173 유지).
