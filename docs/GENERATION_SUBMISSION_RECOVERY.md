...
- `submission-stage` 기능을 claim 요청에 알린다.
...
| 새 서버 + 구 에이전트 | 기존 `submitting` claim을 사용한다. 알려진 구 에이전트의 모호한 `/fail` 보고와 만료는 `recovery_required`로 격리한다. |
| 구 서버 + 새 에이전트 | 응답에 `claim_phase`가 없으면 기존 방식으로 처리한다. 구 서버에는 새 격리 계약이 없으므로 서버 업데이트 전 안전성은 제한적이다. |

운영 적용 순서는 **서버 먼저, 작업자 에이전트 다음**으로 고정한다. 새 서버는 알려진 구 에이전트의
모호한 실패도 격리하지만, 구 서버에는 그 보호 장치가 없으므로 새 에이전트만 먼저 배포해도 RL-05가
완전히 적용됐다고 보지 않는다.
