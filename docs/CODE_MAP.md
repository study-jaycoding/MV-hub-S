...
| `manage/ExportView.tsx`(522줄) | 완료 탭 — **공유 저장 · 최종 저장(골드) │ 비교 · 미러 · 업데이트**. 미러·업데이트는 비교 결과가 있을 때만 켜지고, 저장·프로젝트 변경·새로고침이면 비교를 버린다. 미러는 확인 창(옮길 파일 목록)을 거친다. 위 = 판 하나 · 가운데 = 비교 결과 · 아래 = 저장 대상 ∣ 저장 이력. 설계 `docs/EXPORT_SYNC_DESIGN.md` | |
