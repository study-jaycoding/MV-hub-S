...
from .resolve_python_registry import registry_python_installations
...
def resolve_environment_snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "system_pythons": registry_python_installations(),
    ...
    if connected:
        checks.append(
            _check(
                ...
                "실제 연결",
                "ok",
                connection.get("message", "Resolve 연결됨"),
        ...
        if connection_state == "python_incompatible":
            recommendations.append(
                "64비트 Python(3.9 이상)을 '모든 사용자'용으로 설치하세요. "
                "MV Hub가 자동으로 인식해 Resolve 연결에 사용합니다."
            )
