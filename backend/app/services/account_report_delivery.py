...
def drain_remote_account_reports(
    ...
    *,
    ...
    """전송 가능한 보고 묶음을 보내고 명시적 ACK를 받은 revision만 성공 처리한다."""
    ...
    if invalid_rows:
        repo_manage.mark_account_reports_failed(invalid_rows, "invalid queued report payload")
    ...
    try:
        ...
        pushed = repo_manage.mark_account_reports_pushed(valid_rows)
        return {
            "target": "remote",
            "pushed": pushed,
            "failed": len(invalid_rows),
        }
    except Exception as exc:  # noqa: BLE001 - 큐에 남겨 다음 주기에 재시도
