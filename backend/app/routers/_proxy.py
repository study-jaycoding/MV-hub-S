...
_LOCAL_PREFIXES = (
    ...
    "/api/console/",       # 서버 콘솔 패널(버전·CLI 경로·에이전트/허브 로그) — 이 PC 정보라 서버 위임 금지
...
_LOCAL_EXACT = frozenset(
    {
        ...
        "/api/manage/save-finals/progress",  # 저장이 이 PC 에서 도므로 진행률도 이 PC 가 안다(서버엔 없다)
        "/api/manage/save-finals/cancel",  # 그 저장을 멈추는 것도 이 PC
