...
def notification_domains(method: str, path: str, status_code: int) -> tuple[str, ...]:
    ...
    # 평가하고 경로 분류만 이어간다(종전엔 helper 3개가 각각 재평가 — R4 B-3). 종전 공개
    # helper should_notify_* 3종은 호출자가 없어 2026-08-27 제거했다.
    ...
    if path.startswith("/api/manage/") and path not in {
        "/api/manage/local-task-previews", "/api/manage/project-folders/reveal",
    }:
