"""DB 스키마 마이그레이션 — schema.sql 적용 전후로 도는 멱등 구조 보강.

db.py 에서 분리(순수 재조직 — 동작 변경 없음). conn 만 받아 스키마를 mutate 하는 자족 모듈(sqlite3 만 의존).
init_db(db.py) 가 _pre_migrate → executescript(schema) → _migrate 순으로 호출한다.
"""
