...
# 그룹이 쓸 수 있는 모델(**허용 목록** — 적힌 것만 쓴다). 값은 CLI job_type 문자열.
# ★**빈 목록 = 제한 없음(전부 사용)**. 그래야 그룹을 새로 만들거나 옛 그룹이 그대로 있을 때 아무도 못 만드는 일이 없다.
# 힉스필드 관리 화면은 반대로 'Restricted Models'(차단 목록)다 — Jay 2026-09-11: 화면에서 헷갈리니 우리는 허용 목록으로 간다.
# 힉스필드 CLI·개발자 API 는 워크스페이스별 모델 권한을 돌려주지 않아(2026-09-11 실측) 매니저가 손으로 적는다.
...
def normalize_allowed_models(values: Any) -> list[str]:
    """사용 모델 목록 정규화 — strip·소문자·빈 값 제거·중복 제거(첫 등장 순서) 뒤 형식·개수 검사. 위반은 ValueError(→400)."""
    if values is None:
        return []
    if not isinstance(values, (list, tuple)):
        raise ValueError("사용 모델 목록 형식이 올바르지 않습니다")
    out: list[str] = []
    seen: set[str] = set()
    for raw in values:
        if not isinstance(raw, str):
            raise ValueError("사용 모델 id 는 문자열이어야 합니다")
        model = raw.strip().lower()
        if not model or model in seen:
            continue
        if not _MODEL_ID_RE.fullmatch(model):
            raise ValueError(f"사용 모델 id 형식이 올바르지 않습니다: {raw.strip()[:80]}")
        seen.add(model)
        out.append(model)
    ...
    *,
    ...
    with get_connection() as conn:
        ...
        for order, g in enumerate(groups):
            ...
            if g.get("allowed_models") is None:  # 키 없음 = 기존값 유지(새 그룹은 빈 목록)
...
def my_models(workspace_id: str, email: str) -> dict[str, Any]:
    ...
    with get_connection() as conn:
        _ensure_schema(conn)
        row = conn.execute(
            "SELECT g.id AS group_id, g.name AS group_name, g.allowed_models AS allowed_models, "
    ...
    return {
        ...
        "allowed_models": _parse_allowed(row["allowed_models"]) if row else [],
