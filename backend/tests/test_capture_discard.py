...
def test_commit_capture_token_bookkeeping_is_atomic_with_discard(
    local_only, captures_root, monkeypatch
):
    ...
    with pytest.raises(HTTPException) as exc:  # 신규 시절 토큰은 이제 못 쓴다
        assets.discard_capture(assets.CaptureDiscardIn(token=token), _request())
    assert exc.value.status_code == 404
    assert f.exists()
